...
async function sample(run: (root: string) => Promise<void>) {
  const root = await mkdtemp(path.join(os.tmpdir(), "frontend-chief-content-"));
  try {
    await mkdir(path.join(root, "skills/frontend-chief"), { recursive: true });
    await writeFile(path.join(root, "skills/frontend-chief/SKILL.md"),
      "---\nname: frontend-chief\ndescription: example\n---\n\n# Entry\n");
    await run(root);
