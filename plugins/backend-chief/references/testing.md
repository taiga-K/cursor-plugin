...
テストコードの命名、table/subtest、失敗メッセージ、Helper/Cleanup、fake/mockの選択は[Goのテストの書き方](go/testing.md)を参照する。検証範囲・環境・CIの判断はこの文書に集約する。
...
[Domain実装](domain-implementation.md)と業務の具体例から、変更に関係する観点を選ぶ。

| 対象 | 確認する振る舞い |
|---|---|
| Entity | 同じID・異なる属性と、別ID・同じ属性を区別する |
| VO | 値の等価性、境界、変更前の値の保持。constructor引数やgetterのslice/map/子pointerから内部変更できない |
| 生成・変更 | 初期条件、許可/禁止される遷移、失敗時も不正な途中状態を残さない |
| 再構成 | 保存済みID・状態・履歴を保ち、新規初期化・採番・作成イベントを繰り返さない。破損・旧版データの契約 |
| 集約 | 子の追加・変更・削除もルートのルールを通る。部分更新で不変条件やversionが破れない |
| Application/Domain Service | 実際のDomainの振る舞いとfake Repositoryで判断・調整を検証し、業務判断ごとmockしない |
| Repository Adapter | 選択DBで保存→取得し、IDだけでなく意味のある属性・子・versionのマッピングを確認。未存在・検索条件・競合も対象に応じて確認 |
| Query | 更新→参照の必要な整合性、検索/ページング、認可を確認。参照DTOを保存用集約として流用しない |
| Event | 同期処理と配送を区別し、rollbackで外部に出来事が公開されない。再送で二重適用されない |

再構成を使うfixtureは有効な状態を作り、破損データは破損試験として明示する。IDや時刻を制御し、業務モデルの変更と回帰テストを対応させる。単純な委譲・詰め替えの独立テストは価値で選び、既に確認したマッピングを全検索ケースで繰り返さない。
