---
name: nosql-backend
description: NoSQLを選んだGoバックエンドの永続化を設計・実装・レビューする。製品選定・アクセスパターン・原子単位・整合性・条件付き書込・分割キーを扱う。DBサーバーの構築は対象外。
---
...
1. [DB選択](../../references/database-selection.md)で利用者の選択を確認し、[NoSQL専用資料](../../references/databases/nosql.md)を読む。未採用DBを同時導入しない。
2. 既存の製品・版・ドライバ・schema・索引・実行環境を調べ、既存の設定・設計記録に実際の能力を記録する。NoSQLは製品まで確定して該当製品の資料を読む。
3. 専用資料の手順で製品選定・アクセスパターン・原子単位・整合性・条件付き書込・分割キーを確認し、[整合性](../../principles/consistency.md)を保証する具体的な操作を設計する。
